...
exports.generateBinaryData = generateBinaryData;
exports.generateTimestamp = generateTimestamp;
exports.generateUnixTimestamp = generateUnixTimestamp;
exports.generateTime = generateTime;
exports.generateIpAddress = generateIpAddress;
exports.generateIpv6Address = generateIpv6Address;
exports.generateMacAddress = generateMacAddress;
exports.generateHexColor = generateHexColor;
exports.generateInt = generateInt;
exports.generateFloat = generateFloat;
exports.generateDouble = generateDouble;
exports.generateLong = generateLong;
exports.generateByte = generateByte;
exports.generateByteArray = generateByteArray;
...
exports.generateRandomWithCustomDistribution = generateRandomWithCustomDistribution;
exports.generateRandomPrime = generateRandomPrime;
exports.generateRandomPercentage = generateRandomPercentage;
exports.generateRandomFromSet = generateRandomFromSet;
exports.generateRandomEven = generateRandomEven;
exports.generateRandomOdd = generateRandomOdd;
exports.generateUniqueRandomSequence = generateUniqueRandomSequence;
exports.generateRandomExponential = generateRandomExponential;
exports.generateRandomComplexNumber = generateRandomComplexNumber;
exports.isNumeric = isNumeric;
exports.readAndSaveSchema = readAndSaveSchema;
...
const MonthsAbbreviationsEnum_1 = require("../enums/MonthsAbbreviationsEnum");
const SupportedDateFormats_1 = require("../enums/SupportedDateFormats");
const UUIDEnums_1 = require("../enums/UUIDEnums");
/**
 ...
 */
function generateString(length = Constants_1.Constants.DEFAULT_STRING_LENGTH) {
    const randomBytes = crypto_1.default.randomBytes(length);
    let generatedValue = "";
    for (let i = 0; i < length; i++) {
        const index = randomBytes[i] % Constants_1.Constants.ALPHA_NUM.length;
        generatedValue += Constants_1.Constants.ALPHA_NUM.charAt(index);
    }
 ...
 */
function generateEmail(domain = Constants_1.Constants.DEFAULT_DOMAIN, usernameLength = Constants_1.Constants.DEFAULT_EMAIL_USERNAME_LENGTH) {
 ...
 */
function generatePhoneNumber(countryCode = "FR") {
    // Access the regex pattern from the enum
    // const phonePattern = CountryCodePhoneNumberPatternEnum[countryCode];
    const phonePattern = CountryCodePhoneNumberPatternEnum_1.CountryCodePhoneNumberPatternEnum[countryCode];
    if (!phonePattern) {
    ...
    const randExp = new randexp_1.default(regex);
...
function generateDate(format) {
    ...
    if (format.includes("MM")) {
        ...
        const monthKeys = Object.keys(MonthsAbbreviationsEnum_1.MonthsAbbreviationsEnum);
        const randomIndex = Math.floor(Math.random() * monthKeys.length);
        month = MonthsAbbreviationsEnum_1.MonthsAbbreviationsEnum[monthKeys[randomIndex]];
    }
    // Generate day
    ...
    const dateString = format.replace(/(yyyy)|(yy)|(MM)|(MMM)|(dd)/g, (match) => {
        switch (match) {
            case "yyyy":
 ...
 */
function isSupportedDateFormat(format) {
    return Object.values(SupportedDateFormats_1.SupportedDateFormats).includes(format);
}
/**
 ...
 */
function generateUuid(type = UUIDEnums_1.UuidType.V4, name, namespace) {
    switch (type) {
        case UUIDEnums_1.UuidType.V1:
            return (0, uuid_1.v1)();
        case UUIDEnums_1.UuidType.V3:
            if (!name || !namespace) {
                throw new Error("Namespace and name must be provided for UUID v3");
            }
            return (0, uuid_1.v3)(name, namespace);
        case UUIDEnums_1.UuidType.V4:
            return (0, uuid_1.v4)();
        case UUIDEnums_1.UuidType.V5:
            if (!name || !namespace) {
...
function generateIpv6Address() {
    const getRandomHex = () => Math.floor(Math.random() * 0x10000)
        .toString(16)
        .padStart(4, "0")
        .toUpperCase();
 ...
 */
function generateFloat(minVal = 0, maxVal = 1, decimalPlaces = 2) {
    const randomBytes = crypto_1.default.randomBytes(4);
    const randomValue = randomBytes.readUInt32BE(0) / 0xffffffff;
    return parseFloat((minVal + randomValue * (maxVal - minVal)).toFixed(decimalPlaces));
}
/**
 ...
 */
function generateDouble(min = 0, max = 1, decimalPlaces = 15) {
    const randomBytes = crypto_1.default.randomBytes(8);
    const randomValue = randomBytes.readUInt32BE(0) + randomBytes.readUInt32BE(4) / 0xffffffff;
 ...
 */
function generateLong(min = 0, max = 1000000000) {
    const randomBytes = crypto_1.default.randomBytes(8);
...
function generateByte() {
    return crypto_1.default.randomBytes(1)[0];
}
/**
 ...
 */
function generateShort(minVal = 0, maxVal = 32767) {
 ...
 */
function generateRandomWithCustomDistribution(probabilities) {
 ...
 */
function generateRandomPrime(minVal, maxVal) {
 ...
 */
function generateRandomFromSet(set) {
 ...
 */
function generateRandomEven(minVal, maxVal) {
    let num = generateShort(minVal, maxVal);
    if (num % 2 !== 0) {
        num += 1;
    }
 ...
 */
function generateRandomOdd(minVal, maxVal) {
    let num = generateShort(minVal, maxVal);
    if (num % 2 === 0) {
        num += 1;
    }
 ...
 */
function generateUniqueRandomSequence(minVal, maxVal, length) {
    if (length > maxVal - minVal + 1) {
        throw new Error("Sequence length exceeds the range size.");
    }
    const numbers = Array.from({ length: maxVal - minVal + 1 }, (_, i) => i + minVal);
 ...
 */
function generateRandomExponential(lambdaVal) {
 ...
 */
function generateRandomComplexNumber(realMin, realMax, imaginaryMin, imaginaryMax) {
    const realPart = Math.random() * (realMax - realMin) + realMin;
    const imaginaryPart = Math.random() * (imaginaryMax - imaginaryMin) + imaginaryMin;
 ...
 */
function readAndSaveSchema(filePath) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const jsonContent = yield fs_1.default.promises.readFile(filePath, "utf8");
